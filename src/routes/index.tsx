  ...
  return (
    <div className="absolute inset-0 w-full h-full z-[10] flex flex-col items-center justify-center bg-gradient-to-b from-sky-300 via-sky-100 to-stone-200">
      <div className="flex flex-col items-center animate-pulse">
        ...
        <h2 className="text-xl sm:text-2xl font-black tracking-widest uppercase text-slate-800 mb-3">Rezidencija Vista</h2>
        <div className="w-48 sm:w-64 h-1.5 bg-slate-200 rounded-full overflow-hidden shadow-inner">
          <div className="h-full bg-slate-800 w-[15%] transition-all duration-1000 ease-in-out" />
        </div>
        <p className="mt-4 text-xs font-semibold text-slate-500 uppercase tracking-[0.2em]">Učitavam stanove...</p>
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white flex flex-col font-sans">
      <nav className="flex items-center justify-between px-6 md:px-12 py-6 border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-lg tracking-wide uppercase">Rezidencija Vista</span>
        </div>
        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-neutral-400">
          <a href="#" className="text-white transition-colors">Početna</a>
          <a href="#" className="hover:text-white transition-colors">O Nama</a>
          <a href="#" className="hover:text-white transition-colors">Kontakt</a>
        </div>
        <a href="#stanovi" className="inline-block px-6 py-2.5 rounded-full border border-neutral-700 hover:bg-neutral-800 transition-colors text-sm font-medium">
              ...
              <div className="grid md:grid-cols-3 gap-8">
                ...
                <Link to="/stan/$id" params={{ id: "7d87aaab-ebf3-4d04-9f5f-48930c3ff929" }} className="group block bg-slate-50 rounded-3xl overflow-hidden hover:shadow-2xl transition-all duration-300 ring-1 ring-slate-900/5 hover:-translate-y-1">
                  ...
                  <div className="p-6 space-y-4">
                    <div>
                      <h3 className="text-xl font-bold group-hover:text-amber-500 transition-colors">Sprat 1 / Stan 1</h3>
                      <p className="text-sm text-slate-500 font-medium">Zgrada A · Prednja strana</p>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 border-t border-slate-200 pt-4">
                      <div className="flex flex-col">
                        ...
                        <span className="font-bold text-slate-900">55 m²</span>
                      </div>
                      <div className="flex flex-col border-l border-slate-200 pl-4">
                        <span className="text-xs text-slate-400 uppercase font-semibold">Sobe</span>
                        <span className="font-bold text-slate-900 flex items-center gap-1"><BedDouble className="w-3 h-3 text-slate-400"/> 2</span>
                      </div>
                      <div className="flex flex-col border-l border-slate-200 pl-4">
                        <span className="text-xs text-slate-400 uppercase font-semibold">Kupatila</span>
                        <span className="font-bold text-slate-900 flex items-center gap-1"><Bath className="w-3 h-3 text-slate-400"/> 1</span>
                      </div>
                    </div>
                  </div>
                </Link>
